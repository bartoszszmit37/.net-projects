...
public class bsDataAccessLayer
{
    ...
    /*public bool bs_DalDodajProduktAkcesoria(string bs_nazwa, float bs_cena)
    {
        string bs_zapytanie = "insert into bsTabelaAkcesoria (bsNazwa,bsCena) values(@bsNazwa,@bsCena)";
        SqlParameter[] bs_sql_parameters = new SqlParameter[2];
        bs_sql_parameters[0] = new SqlParameter("@bsNazwa", SqlDbType.VarChar);
        bs_sql_parameters[0].Value = bs_nazwa;
        bs_sql_parameters[1] = new SqlParameter("@bsCena", SqlDbType.VarChar);
        bs_sql_parameters[1].Value = Convert.ToString(bs_cena);
        return bs_polaczenie.bs_IniWykonajDodawanieDoBazy(bs_zapytanie, bs_sql_parameters);
    }*/
