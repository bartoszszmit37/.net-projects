...
public class BusinesLayer
{
    ...
    /*public Wlasciwosci BLWyswietlTabele()
    {
        Wlasciwosci wl = new Wlasciwosci();
        DataTable dt = new DataTable();
        dt = warstwa_dostepu.DalWyswietlwGrid();
        foreach (DataRow dr in dt.Rows)
        {
            wl.NazwaTowaru = dr["NazwaTowaru"].ToString();
            wl.KodTowaru = dr["KodTowaru"].ToString();
            wl.CenaTowaru = Convert.ToDouble(dr["CenaTowaru"]);
        }
        return wl;
    }*/
